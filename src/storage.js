...
export class LocalRepository {
  ...
  load() {
    ...
    if(raw!==null) {try{return validate(JSON.parse(raw));}catch{throw Error('Your saved data could not be read. It has been kept intact. Restore a valid backup or export the saved data for recovery.');}}
    const state=seed(); this.save(state); return state;
  }
